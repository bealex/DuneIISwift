...
public struct UnitScriptRunner: Sendable {
    ...
    func dispatch(_ index: Int, engine: inout ScriptEngine, state: inout GameState, slot: Int) -> UInt16? {
        ...
        switch index {
            ...
            case 0x14: return combat.transportDeliver(slot: slot, in: &state)
            case 0x16: return movement.moveToTarget(slot: slot, engine: &engine, in: &state)
            case 0x18: return unit.findIdle(slot: slot, index: engine.peek(1), in: state)
            case 0x1E: return combat.moveToStructure(slot: slot, type: engine.peek(1), in: &state)
            ...
            case 0x1D: return targets.targetPriority(unitSlot: slot, encoded: engine.peek(1), in: state)
            case 0x2F: return combat.isValidDestination(slot: slot, encoded: engine.peek(1), in: &state)
            case 0x1F: return unit.isInTransport(u)
            case 0x20: return unit.getAmount(u, in: state)
            case 0x21: return combat.randomSoldier(slot: slot, action: engine.peek(1), in: &state)
            case 0x23: return combat.callUnitByType(slot: slot, type: engine.peek(1), in: &state)
