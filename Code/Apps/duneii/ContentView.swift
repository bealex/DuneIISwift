...
struct ContentView: View {
    ...
    var body: some View {
        // No window toolbar at all: the map fills the window beside the sidebar, and every control lives in the
        // sidebar (scenario chooser + speed + debug in Options; Save/Load + Mentat as the lower buttons; pause
        // is the space bar).
        HStack(spacing: 0) {
            mapArea
            GameSidebar(
...
/// sidebar button and the File-menu command (`DuneIIApp.commands`) can invoke the same flow. The game is
/// frozen while the dialog is up (`beginUIPause`/`endUIPause`).
@MainActor func presentSaveGame(_ model: GameModel) {
    model.beginUIPause(); defer { model.endUIPause() }
...
/// Present an open panel and restore the chosen save. Shared by the sidebar button and the File-menu command.
/// The game is frozen while the dialog is up.
