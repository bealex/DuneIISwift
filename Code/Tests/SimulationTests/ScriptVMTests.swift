...
struct ScriptVMTests {
    ...
    func runProgram(_ program: [UInt16], steps: Int,
                    callFunction: @escaping (Int, inout ScriptEngine) -> UInt16? = { _, _ in nil }) -> ScriptEngine {
    ...
    func binaryOps() {
        func eval(_ a: Int, _ b: Int, _ op: Int) -> UInt16 {
            runProgram([inlineOp(PUSH, a), inlineOp(PUSH, b), inlineOp(BINARY, op), bareOp(POP_RET)], steps: 4).returnValue
        }
