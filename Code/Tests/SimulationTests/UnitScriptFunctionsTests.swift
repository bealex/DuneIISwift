...
struct UnitScriptFunctionsTests {
    ...
    private func stateWithUnit(
        _ type: UnitType,
        house: UInt8 = 0,
        at packed: UInt16 = 20 * 64 + 20
    ) -> (GameState, Int) {
        var s = GameState()
        s.playerHouseID = 0
    ...
    func getInfo() {
        ...
        #expect(
            fns.getInfo(slot: slot, field: 0x00, in: &s)
    ...
    func getRandomTile() {
        ...
        let expectTile = Tile32.moveByRandom(
            s.units[slot].o.position,
            distance: 80,
            center: true,
            rng: &probe.random256
        )
    ...
    func findStructure() {
        ...
        #expect(
            fns.findStructure(slot: slot, type: type, in: s)
