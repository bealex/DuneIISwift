...
struct UnitSpriteTests {
    ...
    func harvestingOverlay() throws {
        ...
        let north = try #require(
            UnitSprites.info(
                for: harvester(orientation: 0, action: .harvest, spriteOffset: 2),
                onSpice: true
            )?.overlay
        )
        ...
        for offset in Int8(0) ... 5 {
            let ov = try #require(
                UnitSprites.info(
                    for: harvester(orientation: 0, action: .harvest, spriteOffset: offset),
                    onSpice: true
                )?.overlay
            )
        ...
        let east = try #require(
            UnitSprites.info(
                for: harvester(orientation: 64, action: .harvest, spriteOffset: 0),
                onSpice: true
            )?.overlay
        )
        ...
        #expect(
            UnitSprites.info(for: harvester(orientation: 0, action: .harvest, spriteOffset: 0), onSpice: false)?.overlay
                == nil
        )
        #expect(
            UnitSprites.info(for: harvester(orientation: 0, action: .move, spriteOffset: 0), onSpice: true)?.overlay
                == nil
        )
        #expect(
            UnitSprites.info(for: harvester(orientation: 0, action: .harvest, spriteOffset: -1), onSpice: true)?.overlay
                == nil
        )
    ...
    func everyUnitEightDirections() throws {
        ...
        let v32C4: [(Int, Bool)] = [
            (0, false), (1, false), (1, false), (1, false),
            ...
            for o8 in 0 ..< 8 {
                ...
                if info.movementType == .winger {  // air pass
                    switch info.displayMode {
                        ...
                        case .ornithopter:
                            let (off, fl) = v32E4[o8]; index = base + off * 3 + v33AE[0]; fH = fl & 1 != 0;
                            fV = fl & 2 != 0
    ...
    func rocketDirections() throws {
        ...
        let south = try #require(
            UnitSprites.info(for: unit(.missileRocket, orientation: Int8(truncatingIfNeeded: 128)))
        )
