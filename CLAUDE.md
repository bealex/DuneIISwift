...
- Do not "fix" a scenario-golden divergence before proving it is deterministic. When a golden diverges, first **align the RNG draw stream** (oracle `--parity-random-trace=`/`--parity-lcg-trace=` vs our own draw trace) — the recurring trap is a divergence that *looks* deterministic but is unpinned idle/`wobble` spread (render-only rotation that never touches deterministic state). If the divergence traces to those draws, **gate the scenario at its deterministic horizon and stop** — do not invent a "bug" or patch the math; only a divergence that survives stream-alignment is real. (The `attack-rocket` tick-69 residual cost three wrong diagnoses by skipping this — see insight `sim-rng-stream-unpinned-wobble`.)
