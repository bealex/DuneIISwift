...
**Immediate next step:** the **object pools** (fixed arrays sized to OpenDUNE's `*_INDEX_MAX` — units 102, structures 82, houses 6, teams 16 — plus the dense find-array mirroring `Pool_FindFirst`/`Pool_FindNext` and the `Tools_Index_*` encoding; `Unit_Allocate` picks a free index inside the type's `[indexStart, indexEnd]` band), then **`GameState`** (the single owned aggregate: pools + the `MapTile[64*64]` grid + both RNGs + the two clocks + speed/pause + tick cursors + the hoisted `ObjectInfo.available`). See the "Pools" / "GameState" sections of `Architecture/DataModel.md`. Building the pools unlocks the deferred `Tools_Index_Encode`/`IsValid`/`Get*` primitives; the World-dependent script functions and the EMC transcription (Tier-2a decision traces) follow. Remaining small follow-ups: the `ICM_ICONGROUP_*`/`EXPLOSION_*` enums (currently raw), and a cross-target test reconciling `DuneIIRenderer.StructureCatalog` against `StructureLayoutInfo`.
...
`cd Code && TMPDIR="$PWD/.build/tmp" xcrun swift test --disable-sandbox`: **94 tests, all green** (format/codec/EMC/IconMap + PNG/WAV export + renderer services + **golden parity vs OpenDUNE** (RNG, tile geometry, game-speed, index, stat tables incl. unit/structure info + structure layout + actionsAI, ObjectFlags/MapTile bit layouts) + model-POD defaults, synthetic + real-data). Clean build (`swift package clean` then recreate `.build/tmp`, then `swift build`): zero warnings (full output audited, incl. the `rendertest` product). OpenDUNE oracle: `cd Repositories/OpenDUNE && PATH="$PWD/.shim:$PATH" make -j4` → `./bin/opendune` (the `.shim/gcc`→`xcrun clang` shim is required here; see the build/oracle quirk note above).
