...
**Immediate next step:** the `Unit`/`Structure`/`House`/`Team`/`Map` PODs (faithful to OpenDUNE's `Object`/`Unit`/`Structure`/`House`/`Team`/`Tile` structs for the eventual save-format port), object pools (fixed arrays + find-index), and `GameState` (owns both RNGs, the two clocks, tick cursors). Design-first (workflow step 1): write the model design under `Documentation/Architecture/` before code, since these are harder to golden-test in isolation than the constant tables. That unlocks the deferred pool-dependent primitives (`Tools_Index_Encode`/`IsValid`/`Get*`) and the World-dependent script functions; the EMC transcription follows, verified by Tier-2a decision traces. Remaining small follow-ups: the `ICM_ICONGROUP_*`/`EXPLOSION_*` enums (currently raw), and a cross-target test reconciling `DuneIIRenderer.StructureCatalog` against `StructureLayoutInfo`.
...
`cd Code && TMPDIR="$PWD/.build/tmp" xcrun swift test --disable-sandbox`: **90 tests, all green** (format/codec/EMC/IconMap + PNG/WAV export + renderer services + **golden parity vs OpenDUNE** (RNG, tile geometry, game-speed, index, stat tables incl. unit/structure info + structure layout + actionsAI), synthetic + real-data). Clean build (`swift package clean` then recreate `.build/tmp`, then `swift build`): zero warnings (full output audited, incl. the `rendertest` product). OpenDUNE oracle: `cd Repositories/OpenDUNE && PATH="$PWD/.shim:$PATH" make -j4` → `./bin/opendune` (the `.shim/gcc`→`xcrun clang` shim is required here; see the build/oracle quirk note above).
