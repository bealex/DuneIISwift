...
- **Tile geometry + orientation.** `Tile32` (pack/unpack, distance/direction) + `Orientation` in `DuneIIWorld/Tile/`, golden-verified (`TileGoldenTests`, 7 tests). Per-category golden fixtures (`--parity-golden=<dir>` → `rng-golden.jsonl` / `tile-golden.jsonl`) + shared `GoldenFixture` loader. Doc `Algorithms/Tile.md`. See History 2026-05.
