...
`Code/Tests/WorldTests/Fixtures/rng-golden.jsonl` — committed output of `opendune --parity-golden=<path>`. Covers `Tools_Random_256` over seeds `{0, 1, 0xC0DE, 0x12345678, 0xFFFFFFFF}` (256 bytes each) and `Tools_RandomLCG_Range` over seeds `{0, 1, 0x1234, 0x7FFF}` × ranges `{[0,32767], [0,1], [0,100], [1,6], [5,5], [100,0], [0,255]}` (64 values each; `[100,0]` exercises the swap, `[5,5]` the degenerate span). Regenerate: rebuild the patched OpenDUNE (`FunctionParityHarness.md`) and re-run the flag; recommit the file. Asserted by `WorldTests/RngGoldenTests`.
