...
- **Slice 3 (shipped 2026-04-23) — briefing text**: decodes the English string table via a new `Formats.Strings.decode(_:compressed:)` port of OpenDUNE's `String_DecompressAndTranslate` (`src/string.c:42..81`), loads `TEXT{H,A,O}.ENG` for the player house, and picks the local string at `campaignID * 4 + 4` (mission-1 = index 4). Rendered at the bottom of the mentat screen as a word-wrapped `SKLabelNode`. The "click to continue" caption is only shown as a fallback when the install can't supply the string. `MentatScene.fetchBriefingText(fromLoader:playerHouseID:campaignID:)` is the testable resolver. **No voice**: OpenDUNE's `GUI_Mentat_ShowDialog` explicitly calls `Driver_Voice_Play(NULL, 0xFF)` to stop any playing voice (`src/gui/mentat.c:102`); the Dune II briefings are text-and-music only. Briefing-music playback (`Music_Play(houseInfo.musicBriefing)` at `mentat.c:104`) is still a follow-up — the jukebox currently starts `DUNE0.XMI` on scenario entry; wiring a briefing-specific track would need an `XmiTrack` lookup.
