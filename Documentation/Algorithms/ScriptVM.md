...
`ScriptInfo` + `ScriptInterpreter` (protocol) + `DefaultScriptInterpreter` live in `DuneIISimulation/Script/`, injected into `Simulation` like the other replaceable primitives — which also gives the parity harness's instrumented/tracing variant a home (OpenDUNE traces `Script_Run` via `g_parityScriptTrace`). `Script_Reset`'s field resets stay on the `ScriptEngine` POD (`reset()`, World) since they're pure state. Op-14 dispatch is a `(Int, inout ScriptEngine) -> UInt16?` closure passed to `run`; the real per-category native tables (which need `GameState` + the current object) wire through it when they land. In OpenDUNE the table is fully populated (unused slots = `NoOperation`), so a function call never suspends — only `Delay` does. We mirror that: the closure returns a value and execution continues, or returns `nil` for a **not-yet-ported** native, in which case `run` **halts the script cleanly** (nulls the PC) so the unit freezes loudly instead of silently no-oping or — as an earlier version did — keeping the PC and resuming past the call next tick (which skewed tick timing).
