...
## Team creation (`Team_Create` + the `[TEAMS]` scenario section)

`GameState.teamCreate(houseID:teamActionType:movementType:minMembers:maxMembers:scriptPC:)` (`GameState+Pools`, port of `Team_Create` `team.c:75`) allocates a team (`teamAllocate`), sets its identity + membership bounds (`action == actionStart` so `Load2` can restore it), and loads its action script (`Script_Reset` + set `scriptPC`, `delay = 0`). Because the team script *program* lives in the Simulation layer, World can't index it — the caller passes the resolved `scriptPC` (the team `ScriptInfo`'s `offsets[teamActionType]`), or `ScriptEngine.scriptNull` to leave the team inert (`GameLoop_Team` won't run an unloaded script). The PC is set once at scenario-load time and persists in `state.teams[…].script` (so it survives the per-tick `Simulation` reconstruction — it must **not** be reloaded in `Simulation.init`).

The scenario loader parses `[TEAMS]` lines `"<House>,<TeamAction>,<MovementType>,<minMembers>,<maxMembers>"` (`Scenario_Load_Team` `scenario.c:410`) via `loadScenario(…, teamScriptOffsets:)` → `loadTeam` → `teamCreate`, using the new `MovementType.named`/`TeamActionType.named` string parsers (`g_table_movementTypeName` / `g_table_teamActionName`; note the INI spells winger movement "Winged"). Scenarios without `[TEAMS]` (or without offsets) are unaffected.
