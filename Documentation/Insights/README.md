...
- [host-presentation-gap-not-sim](host-presentation-gap-not-sim.md) — a "missing" cue/visual in the `duneii` client is almost always a host gap (a `VoiceTable` mapping, a `FrameInfo` field, an overlay node), not a sim bug — the sim usually already emits the data; check the host first.
- [render-fog-applies-to-every-overlay](render-fog-applies-to-every-overlay.md) — fog isn't one step: every layer that draws world content (terrain, sprites, minimap base+blips, worm shimmer) re-applies the `isUnveiled` mask itself; a new overlay must thread `showFog` through too.
- [duneii-speed-multiplier-is-a-tick-rate-timewarp](duneii-speed-multiplier-is-a-tick-rate-timewarp.md) — the toolbar 0.5×–4× (`GameModel.gameSpeed`) only scales sim ticks/sec; it's distinct from OpenDUNE's `state.gameSpeed` cadence config. Anything tick-driven (build, movement, combat) scales for free — don't special-case the multiplier.
...
- [sim-player-build-onhold-no-autoresume](sim-player-build-onhold-no-autoresume.md) — the player's out-of-money construction never auto-resumes (only the repair pad does; the AI never holds); the player clicks the held item to resume (`STR_ON_HOLD`). Don't "fix" it with an auto-resume — that's a parity divergence.
- [render-throttle-neutral-default](render-throttle-neutral-default.md) — a per-frame presentation throttle stays golden-neutral by defaulting its interval to every-frame (1) and having only the host opt into a coarser cadence; the goldens render the default path and stay byte-identical (no regen).
...
- [swift-observable-didset-self-assign](swift-observable-didset-self-assign.md) — assigning a property inside its own `@Observable` `didSet` re-enters the setter and recurses forever; clamp at the call site, not in `didSet`.
