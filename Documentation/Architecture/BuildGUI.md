...
### Query — `Simulation.buildOptions(forStructure:)`
The **full** build menu for the GUI listing: every item the house could ever build at this factory (filtered only by `availableHouse`, with the construction-yard self-entry — `FLAG_STRUCTURE_NEVER` — excluded), each as a `BuildOption` = the `Buildable` plus a `[BuildBlocker]` listing why it isn't buildable yet. A `BuildBlocker` is `.structure(StructureType)` (a missing prerequisite), `.campaign(level:)` (the construction-yard `availableCampaign - 1` gate the active `campaignID` hasn't reached — units have no campaign gate), or `.upgradeLevel(Int)` (the factory needs upgrading first); credits are **not** a blocker (they change every tick — the UI checks them live). `blockers` empty ⇔ buildable now, so the available subset (`option.isAvailable`) is exactly what `buildables(forStructure:)` returns — `buildOptions` just adds the locked rows for the greyed-out listing. Same gating as `Structure_GetBuildable` (`structure.c:1834`), but always from the player's perspective (no AI/non-player prerequisite bypass — that path stays in `buildables`, which the AI uses). The `duneii` build panel renders every option, greying locked ones with a "Requires: …" tooltip; the **campaign level** that gates them is derived from the loaded scenario's mission number (`ScenarioID.campaign` → `state.campaignID`), since our scenario `.INI`s don't carry it. The scenario chooser (`ScenarioPicker` popover) groups scenarios by house (tabs) then campaign level, so picking a mission sets the matching tech level automatically.
