...
`Simulation.advanceUnitCadence()` computes which fired this tick (a `UnitTickFlags` set) and advances the cursors, faithfully. The **per-unit work** each flag drives (movement tick, rotation, script step, deviation decay, …) is the unit state machine — now ported (movement + combat + the EMC script). **`GameLoop_Structure` and `GameLoop_House` are now live too**, each with its own cursors (`StructureTickCursors` / `HouseTickCursors` in `GameState`): the structure phase runs the per-structure BUILD.EMC script (turrets, refinery, death) **and** the build/repair economy (`structureTickStructure`: structure-repair + factory production + the repair pad) on the structure cursor — the upgrade branch + AI maintenance remain seams; the house phase runs the core economy (`House_CalculatePowerAndCredit`, `Structure_CalculateHitpointsMax`, the credit clamp, the power upkeep). **`GameLoop_Team` is now live too** — its single random-period cursor (`teamLoopTick`, 5–12 ticks) walks every `isAIActive` house's team and runs one opcode of its `TEAM.EMC` script; the team "brain" natives (recruit/target/order) are seams pending follow-up slices. See `Algorithms/StructureScript.md` (structure phase) and `Algorithms/TeamScript.md` (team phase).
