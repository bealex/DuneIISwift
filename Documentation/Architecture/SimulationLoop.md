...
`Simulation.advanceUnitCadence()` computes which fired this tick (a `UnitTickFlags` set) and advances the cursors, faithfully. The **per-unit work** each flag drives (movement tick, rotation, script step, deviation decay, …) is the unit state machine — now ported (movement + combat + the EMC script). **`GameLoop_Structure` and `GameLoop_House` are now live too**, each with its own cursors (`StructureTickCursors` / `HouseTickCursors` in `GameState`): the structure phase runs the per-structure BUILD.EMC script (turrets, refinery, death) — the BUILD/REPAIR/factory production sub-tick is still a seam; the house phase runs the core economy (`House_CalculatePowerAndCredit`, `Structure_CalculateHitpointsMax`, the credit clamp, the power upkeep). **`GameLoop_Team` remains an order-preserving stub** (teams/AI not yet ported). See `Algorithms/StructureScript.md` for the structure phase.
