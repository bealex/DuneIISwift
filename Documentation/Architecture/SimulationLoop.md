...
`Simulation.advanceUnitCadence()` computes which fired this tick (a `UnitTickFlags` set) and advances the cursors, faithfully. The **per-unit work** each flag drives (movement tick, rotation, script step, deviation decay, …) is the unit state machine — now ported (movement + combat + the EMC script). **`GameLoop_Structure` and `GameLoop_House` are now live too**, each with its own cursors (`StructureTickCursors` / `HouseTickCursors` in `GameState`): the structure phase runs the per-structure BUILD.EMC script (turrets, refinery, death) **and** the full build/repair/upgrade economy (`structureTickStructure`: structure-repair + factory production + the repair pad + the upgrade branch via `Structure_IsUpgradable`) on the structure cursor — only the AI-maintenance block remains a seam; the house phase runs the core economy (`House_CalculatePowerAndCredit`, `Structure_CalculateHitpointsMax`, the credit clamp, the power upkeep). **`GameLoop_Team` is now live too** — its single random-period cursor (`teamLoopTick`, 5–12 ticks) walks every `isAIActive` house's team and runs one opcode of its `TEAM.EMC` script; the getter + **recruit/target** brain natives (`AddClosestUnit`/`GetAverageDistance`/`FindBestTarget`) are live, the order-issuing natives (`Unknown0543`/`Unknown0788`) + `Load`/`Load2` + team creation remain seams. See `Algorithms/StructureScript.md` (structure phase) and `Algorithms/TeamScript.md` (team phase).
