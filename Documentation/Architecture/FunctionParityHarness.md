...
- ⏭ Next: begin the World model — `DuneIIContracts` seam enums + `DuneIIWorld` PODs (`Unit`/`Structure`/`House`/`Team`/`Map`), object pools, and the static stat-table port from `src/table/*info.c`. That unlocks the deferred pool-dependent primitives (`Tools_Index_Encode`/`IsValid`/`Get*`) and the World-dependent script functions.
