...
4. **Blank lines** — placements swift-format has no option for (it never *adds* blank lines). Inside braced bodies / closures: a nested (local) `func` is surrounded by a blank line, and a `guard` is followed by a blank line — with a run of consecutive guards kept together and the single blank line only after the last one. Inside a type's member block: each function-like member (method / `init` / `deinit` / `subscript`) is surrounded by a blank line, while adjacent stored properties stay grouped. It only changes the blank count on a declaration that already starts its own line and preserves leading comments and indentation.
