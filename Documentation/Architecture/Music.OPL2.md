...
- **⚠️ Build optimized.** OPL3 FM synthesis renders at **~1.9× real-time in a debug build** (it can't keep up — music underruns/glitches) and **~0.17× in release** (6× headroom). Run the client optimized for music: **`swift run -c release duneii`**. The rest of the engine is deterministic/headless and fine in debug; only the FM synth needs `-O`.
