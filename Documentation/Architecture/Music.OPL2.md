...
- **Dependency:** `https://github.com/bealex/SwiftOLP3.git`, currently `branch: "main"` (the synth is being actively optimised ahead of a tagged release; `Package.resolved` pins the exact commit so builds stay reproducible — switch back to `from: "1.x"` once a release ≥ the speedup is tagged). The author's URL is the SSH form `git@github.com:bealex/SwiftOLP3.git`; we resolve the same repo over HTTPS so it works without SSH in sandboxed/CI builds. Transitively pulls `memoirs-ios`, which `SwiftOPL3` only *compiles in* under its `OPL_TRACE` trait (off here). Wired into `DuneIIAudio` only.
- **Player:** `Code/Frameworks/DuneIIAudio/ADLMusicPlayer.swift` — a **producer/consumer** design. A background **synthesis thread** runs the `OPL3Chip` + `ADLPlayer` driver (ticked at 72 Hz; the `adlrender` host loop) and fills a PCM **ring buffer** (~1 s lookahead); the `AVAudioSourceNode` callback only copies frames out of the ring (real-time-safe) or emits silence on underrun. OPL3 emulation is far too heavy to run *in* the render callback. All cross-thread state lives in `Mutex`es inside a `Sendable` box, the chip/driver are confined to the producer thread, and the render closure captures no `self` (nonisolated `@Sendable`). No `nonisolated(unsafe)`. See insight `swift-audio-render-thread-mutex`.
  - **Gapless rollover.** A non-looping (ambient/attack) track fires the director's end-of-track callback **once** when its driver reaches the loop point, but the producer keeps generating (the Westwood channels are now repeating, so it loops seamlessly) until the next track's command supersedes it. So the music never falls silent while the (main-actor) `onFinished → advanceAmbient → play` rollover resolves — even if the main thread is briefly busy. (Earlier it *stopped* producing on end and waited, which went silent for the round-trip — seconds under a busy main thread.)
- **Build mode / throughput.** After the upstream SwiftOPL3 synth optimisation (now tracking `main`, commit `0cc22ab`), OPL3 FM synthesis renders at **~0.67× real-time in a debug build** (it now keeps up — the producer thread sustains playback) and **~0.04× in release** (25× headroom). Release is still recommended for margin (`swift run -c release duneii`), but debug playback no longer underruns. *(Pre-optimisation, for the record: debug was ~1.9× real-time — couldn't keep up — and release ~0.17×.)*
