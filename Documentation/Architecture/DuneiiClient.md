...
- **Left-click** — selects the unit/structure under the cursor (`pick`). On **bare ground** (nothing selectable) it instead **inspects that tile** (see *Tile inspector*). While an order is armed, the left-click supplies the target.
- **Right-click** — issues the selected unit's **default contextual order**: a **harvester** → `Harvest` (its `actionsPlayer[0]`; it can't attack), else an **enemy** tile → `Attack`, else → `Move`. (`InputController.rightClick(enemyTarget:harvester:)`, the facts resolved host-side.)
