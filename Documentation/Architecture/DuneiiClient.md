...
- **Left-drag** — **drag-select a group**: rubber-bands a box (green) and selects every player-owned, on-map, normal unit inside it, then (on release) reduces the box to its **dominant unit type** (the most-numerous type; ties → lowest type id) — `GameModel.dragSelect` → `InputController.dominantGroup` → `selectGroup`. A mixed group (e.g. trike + harvester) can't share one order, so only the majority type is kept. Orders apply to the whole group; each selected unit gets a white outline, and the inspector header shows `×N`. Suppressed while a mode is active. *(The original selects one unit at a time — group select is a client convenience.)*
...
- **Keyboard action shortcuts** — `a`/`m`/`h` arm a targeted Attack/Move/Harvest (crosshair, next click is the target); the immediate actions apply at once: `r` Return, `e` Retreat, `g` Guard, `d` Deploy, `b` Sabotage, `x` Destruct; `s` always Stops. A key fires **only** if the action is in the selected unit's `actionsPlayer` (so `a` is ignored for a harvester, `r` for a tank) — `GameModel.issueAction`. `Esc` cancels the active mode else deselects; `+`/`-` zoom; arrows scroll. The inspector buttons show the same letters.
- **Save / load** — the toolbar **File menu** ("Save Game…" / "Load Game…") writes/restores our `SaveGame` (a versioned binary plist of the whole `GameState`, a bit-identical deterministic resume) via `NSSavePanel`/`NSOpenPanel` → `GameModel.saveGame(to:)`/`loadGame(from:)`.
