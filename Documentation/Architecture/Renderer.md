...
- **`SpriteKitRenderer`** *(the visual leaf — implemented).* Colorizes the terrain buffer (`IndexedImage` + the palette / `PaletteAnimator`) into one nearest-filtered `SKSpriteNode` — recomposited only when the tiles change, recoloured on a palette-cycle tick change — and each `ComposedSprite` (house-recoloured via `HouseRemap`, index 0 transparent) into a sprite node rebuilt every frame, in a camera-scroll/zoomable scene. `@MainActor`; realizes the `Renderer` seam shape (`render(_:)` per frame) without a formal conformance, so the headless seam stays actor-agnostic. **Hosted in `mapview`:** `MapScene` now advances the `Simulation`, snapshots `makeFrameInfo()`, and hands it to the renderer each frame (replacing the old `MapImageBuilder` that reached into `GameState`); the app-side `MapSpriteSource` provides the assets. This proves the seam end-to-end on real `SCEN*.INI` scenarios.
