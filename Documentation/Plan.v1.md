...
**Net:** the movement cluster (D′1–D′3 + driver) and the read part of Tier E (#16) + `Unit_Damage` (#18) are done. **Recommended next: `Unit_Deviate` (#17), then the combat-integration slice — `Script_Unit_Fire`'s projectile path + `Map_MakeExplosion` (#15) + `Unit_EnterStructure` — which close the `Unit_Move` SEAMs and let the attack/guard scenario goldens (generated via the oracle) raise their `comparedTicks`.**
