...
**Net:** the movement cluster (D′1–D′3 + driver), the read part of Tier E (#16), `Unit_Damage` (#18), `Unit_Deviate` (#17), and `FindBestTarget` are done. **Recommended next: the fog/visibility model (`Tile_RemoveFogInRadius` + the per-house `seenByHouses` reveal in `Unit_UpdateMap`) — combat AI (`FindBestTarget`) is gated on it, so the attack-scenario goldens can't extend past the deterministic prefix until units' visibility matches the oracle. Then the combat-integration slice — `Script_Unit_Fire`'s projectile path + `Map_MakeExplosion` (#15) + `Unit_EnterStructure` — which is behavioural (RNG), not byte-exact.**
