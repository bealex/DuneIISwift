...
19. `Unit_SetDestination` / `Unit_SetAction` (`unit.c:497`) / `Unit_Hide` — pools ✓, Tiers B–D. **EMC VM core now done** (`DuneIISimulation/Script/`: `ScriptInfo` + `ScriptInterpreter`/`DefaultScriptInterpreter` = `Script_Run` + stack + `Load`/`LoadAsSubroutine`/`Reset`/`IsLoaded`, doc `Algorithms/ScriptVM.md`, `ScriptVMTests`). Remaining for `Unit_SetAction`: a real EMC→`ScriptInfo` bridge for the unit category + the `g_scriptFunctionsUnit` native table (op-14 dispatch closure).
