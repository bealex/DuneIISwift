...
- 17. `Unit_Deviate` (`unit.c:1241`) — **done** (`UnitCombat.deviate`): eligibility guards (normal/not-deviated/deviatable), `toughness`-derived probability (−⅛ for non-player), one `Random256` draw, then `deviated=120`/`deviatedHouse` + action flip + untarget. `Unit_UpdateMap(2)` render seam. `UnitCombatTests` (4).
- 18. `Unit_Damage` (`unit.c:1530`) — **done** (`DuneIISimulation/UnitCombat.swift` `damage`): HP drain, `Unit_Deviation_Decrease`, death → `Unit_RemovePlayer` (new `GameState.unitRemovePlayer`) + `Unit_SetAction(DIE)`, ambush→attack, sandworm/half-HP death, infantry→soldier downgrade (RNG retreat), tracked/wheeled/harvester smoke. Seams: `Map_MakeExplosion` (#15, `range != 0`), `Map_FillCircleWithSpice` (harvester death), `Sound_Output_Feedback` (audio), `Unit_UpdateMap(2)` (render). Decision-trace tests `UnitCombatTests`. Not yet wired into `Unit_Move`'s degrades branch (deferred to the combat-integration slice to avoid a Move↔Combat cycle).

- `Unit_FindBestTargetEncoded` (`unit.c:2396`) + `Unit_FindBestTargetUnit`/`Structure` + `Unit_GetTargetUnitPriority`/`StructurePriority` — **done** (`DuneIISimulation/TargetFinder.swift`, native `Script_Unit_FindBestTarget` op-`0x1C`): deterministic auto-target by priority/distance, gated by `seenByHouses`/alliance/`targetAir`. `TargetFinderTests` (4). Exercised by the guard/attack scenario goldens.

- **Fog of war — `Map_UnveilTile` / `Tile_RemoveFogInRadius` / `Unit_RemoveFog`** (`GameState+Fog.swift`): the player's `isUnveiled` tiles + the occupant `seenByHouses` reveal. Player-only (no-op for AI houses); render dirty-marking is a seam. `FogTests` (3). **Ported but NOT wired into the `Script_Unit_RemoveFog` native yet** — revealing an enemy changes `FindBestTarget`'s visibility mid-combat, which diverges the attack goldens earlier until the full combat matches the oracle. Flip `removeFog` to call `unitRemoveFog` when combat lands.

**Net:** the movement cluster (D′1–D′3 + driver), the read part of Tier E (#16), `Unit_Damage` (#18), `Unit_Deviate` (#17), `FindBestTarget`, and the fog primitives are done. **Combat-integration slice — step 1 done:** the GUARD-loop natives the script clean-halted on are ported — `Script_Unit_SetDestination` (`0x05`, + the shared `Unit_SetDestination` primitive home), `Script_Unit_FindStructure` (`0x25`), `Script_Unit_IdleAction` (`0x31`) in `UnitScriptFunctions`. A guarding unit now loops end-to-end (…→`FindBestTarget`→…→`IdleAction`→`Delay`) and acquires targets. `IdleAction`'s stochastic idle twitch drifts the `random256` stream from the oracle's unpinned render-only `wobble` draws (which we don't chase, Principle 1), so the `guard` golden regated to its deterministic prefix (`compared: 6`); insight `sim-rng-stream-unpinned-wobble`. **Step 2a done:** the ATTACK script clean-halted on `Script_General_GetDistanceToObject` (`0x3E`) — ported (`GeneralScriptFunctions.getDistanceToObject` → `Object_GetDistanceToEncoded`, structure→facing-edge via `StructureLayoutInfo.edgeTiles`); **`attack-close` matched through tick 60**. **Step 2b done:** ported `Unit_Create` + `Unit_IsTileOccupied` + `Unit_CreateBullet` + the full `Script_Unit_Fire` firing path (`UnitCombat.swift`; dispatch `0x08` → `combat.fire`) + extracted `Unit_SetTarget` (`UnitScriptFunctions.unitSetTarget`). **`attack-close` matched through tick 66** (`compared: 67`). **Step 2c done:** ported `Map_MakeExplosion` (#15) + relocated `Unit_Damage` to the movement layer (`UnitImpact.swift`, breaking the Move↔Combat cycle) + wired the three `Unit_Move` bullet-impact points. **`attack-close` now matches the oracle for the FULL 400 ticks** (`compared: 0`): fire → bullet flight → impact damage (defender 200→188, GUARD→HUNT) → retaliation (attacker 200→188), bit-identical. **🎯 Full unit-vs-unit combat parity.** **Recommended next (step 2d/2e): `Unit_EnterStructure` (`unit.c:2177`, the structure-arrival SEAM in `Unit_Move`); flip `Script_Unit_RemoveFog` (`0x28`) on now that combat matches; `Structure_Damage` (the `Map_MakeExplosion` structure SEAM) so bullets damage buildings; a longer-range / `notAccurate`-missile scenario.** Combat that draws RNG while units are stationary stays byte-exact (no wobble desync); add the behavioural bar only where idle/wobble RNG enters.
...
19. `Unit_SetDestination` / `Unit_SetAction` (`unit.c:497`) / `Unit_Hide` — pools ✓, Tiers B–D. **EMC VM core done** (`DuneIISimulation/Script/`: `ScriptInfo` + `ScriptInterpreter`/`DefaultScriptInterpreter`, doc `Algorithms/ScriptVM.md`). **Natives in progress** (clean explicit-param functions, heavily tested): `GeneralScriptFunctions` (12), `UnitScriptFunctions` (11 + `getInfo`), `Unit_FindClosestRefinery`. **`Unit_SetAction` core done** (`DuneIISimulation/UnitActions.swift`, `UnitActionsTests`). Remaining for #19: the real EMC→`ScriptInfo` bridge (unit category) + the op-14 dispatch glue wiring the `g_scriptFunctionsUnit` table (incl. the thin `Script_Unit_SetAction` wrapper).
...
21. pathfinder: `Script_Unit_Pathfinder` + `_Connect` + `_Smoothen` (`script/unit.c:1183,1117,1012`) — **done** (`DuneIISimulation/Pathfinder.swift`; direct-line + wall-follow + smoothen, cost via `tileEnterScore`). Golden vs the oracle (`Golden_Pathfinder` → `pathfinder-golden.jsonl`, `PathfinderTests`). `Unit_GetTileEnterScore` (16) ✓, `Tile_*` ✓.
